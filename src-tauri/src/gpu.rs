...
fn verify_candidate(job: &MiningJob, nonce: u64) -> bool {
    ...
    leading_zero_bits(&digest) >= job.difficulty_bits
}
...
pub fn start_gpu_mining(
    ...
    Some(thread::spawn(move || {
        pollster::block_on(gpu_mine_loop(job, power_percent, stop_flag, hash_count, tx));
    }))
}
...
async fn gpu_mine_loop(
    ...
    let zero_found = GpuFound { flag: 0, nonce_hi: 0, nonce_lo: 0, _pad: 0 };
...
mod tests {
    ...
    fn gpu_search_matches_cpu_sha256() {
        ...
        let handle = start_gpu_mining(job, power_percent, Arc::clone(&stop_flag), hash_count, tx)
